...
import { Calendar, Phone, Hash, Pencil } from "lucide-react"
...
export default async function ProfilePage() {
  ...
  const infoRows = [
    { label: "User ID", value: user.id,       icon: <Hash className="h-3.5 w-3.5" />,     truncate: true },
    { label: "Joined",  value: new Date(user.createdAt).toLocaleDateString("en-GB", { day: "numeric", month: "long", year: "numeric" }), icon: <Calendar className="h-3.5 w-3.5" /> },
    { label: "Phone",   value: user.phone ?? null, icon: <Phone className="h-3.5 w-3.5" />, fallback: "Not provided" },
  ]
      ...
      <div className="w-full max-w-115 bg-white border border-black/[0.07] rounded-[28px] overflow-hidden shadow-[0_20px_60px_rgba(0,0,0,0.1),0_4px_12px_rgba(0,0,0,0.06)]">
        ...
        <div className="px-8 pb-8 pt-3">
          ...
          <div className="space-y-2.5">
            {infoRows.map(({ label, value, icon, truncate, fallback }: any) => (
              <div key={label} className="flex items-center justify-between gap-3 bg-[#faf7f3] rounded-xl px-4 py-2.5">
